test_that("Demean sucessfully removes mean", {
  ...
  classes <- base::cbind(1:20000, c(base::rep(0, 10000), base::rep(1, 10000)))
...
test_that("CILP works when all negative",{
  ...
  dat <- rbind(rmvnorm(1000, mean = 1:10, sigma = cov_mats[, , 1]), rmvnorm(1000, mean = 10:1, sigma = cov_mats[, , 2]))
  colnames(dat) <- LETTERS[1:10]
  rownames(dat) <- 1:2000
...
test_that("Eigengene discovers mean differences",{
  ...
  clustering_check <- all(clust_res == c(rep(1,100),rep(2,100))) | all(clust_res == c(rep(2,100),rep(1,100)))
