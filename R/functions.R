...
#' obs <- rbind(sapply(1:10,function(x){rnorm(100,mean=x)}), sapply(10:1,function(x){rnorm(100,mean=x)}))
#' 
...
#' 
#' This function generates a matrix where the columns/features are the pairwise products of the input matrix features. The new matrix will retain the same number of rows/observations, but have \textit{m} choose 2 columns for each pair of features. When using this function where the mean has been removed, the feature/column means become observations of covariance.
#'
#' @param mat A matrix with observations (e.g. individuals) as rows and features as columns.
#' @param delimiter A delimiter for the pairwise feature names.
#'
#' @return A matrix with all pairwise interactions (pairwise column/feature products) from the input matrix. Column names will show what features were used for the pariwise product.
#' @export
#'
#' @examples
...
#' centered_dat <- scale(dat,scale=F)
...
#'
#' This function generates a matrix where the columns/features of the input matrix features have been squared. The new matrix will retain the dimensions of the input matrix. On a centered matrix, this function will turn the observations into observations of variance.
#'
#' @param mat A matrix with observations (e.g. individuals) as rows and features as columns.
#'
...
#' 
#' # Center matrix
#' centered_obs <- scale(obs,scale=F)
#' 
#' # Generate squared matrix
...
#' 
#' Runs an efficient implementation of CILP from Lea et al. 2019. CILP detects differential correlation of pairwise features between two groups. 
#'
#' @param prod_mat A matrix with all class-specific mean-removed pairwise interactions to be tested.
#' @param groupings A numeric vector denoting class/group membership of the observations.
#'
#' @return A data frame of effect size estimates and p-values for each feature pair.
#' @export
#'
#' @examples
#' # Generate covariance matrix
